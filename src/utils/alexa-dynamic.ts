...
export async function getAccountInfo(env: Env) {
	...
	if (cached) {
		console.log(`[Alexa] Using cached account info (Customer ID: ${cached.customerId.substring(0, 8)}...)`);
		return cached;
	}

	console.log("[Alexa] Connecting to Amazon Alexa API...");
	...
	if (!response.ok) {
		...
		console.error(`[Alexa] ❌ Connection failed: ${response.status} ${response.statusText}`);
		console.error(`[Alexa] Error details: ${errorText.substring(0, 200)}`);
		
		if (response.status === 401 || response.status === 403) {
			console.error("[Alexa] ⚠️  Authentication failed. Possible causes:");
			console.error("[Alexa]   1. Cookies may have expired - get fresh ones from Amazon");
			console.error("[Alexa]   2. Wrong cookie values - make sure you copied the VALUES, not names");
			console.error("[Alexa]   3. Regional cookies may not work - try getting cookies from amazon.com (US)");
		}
	...
	if (!primaryAccount) {
		console.error("[Alexa] ❌ No account found in response");
	...
	console.log(`[Alexa] ✅ Successfully authenticated! Customer ID: ${accountInfo.customerId.substring(0, 8)}...`);
...
export async function getAlexaEndpoints(env: Env) {
	...
	if (cached) {
		console.log(`[Alexa] Using cached endpoints (${cached.length} endpoints)`);
		return cached;
	}

	console.log("[Alexa] Fetching smart home endpoints...");
	...
	if (!response.ok) {
		console.error(`[Alexa] Failed to fetch endpoints: ${response.status} ${response.statusText}`);
		throw new Error(`Failed to fetch endpoints: ${response.status}`);
	}

	const data = await response.json();
	const validatedData = EndpointsDiscoverySchema.parse(data);
	const endpoints = validatedData.endpoints;
	console.log(`[Alexa] ✅ Found ${endpoints.length} smart home endpoints`);
...
export async function getAlexaDevices(env: Env) {
	...
	const cached = getCached<any[]>(cacheKey);
	if (cached) {
		console.log(`[Alexa] Using cached devices (${cached.length} devices)`);
		return cached;
	}

	console.log("[Alexa] Fetching devices from Amazon API...");
	...
	if (!response.ok) {
		const errorText = await response.text().catch(() => "");
		console.error(`[Alexa] Failed to fetch devices: ${response.status} ${response.statusText}`);
		console.error(`[Alexa] Error details: ${errorText.substring(0, 200)}`);
	...
	// Extract devices - API response structure may vary, so handle it flexibly
	let devices: any[] = [];
	...
	if (data.devices && Array.isArray(data.devices)) {
		...
		console.warn("[Alexa] Unexpected API response structure:", Object.keys(data as Record<string, unknown>));
		devices = [];
	}
	
	// Try schema validation but don't fail if it doesn't match
	try {
		const validatedData = PhoenixDevicesSchema.parse({ devices });
		devices = validatedData.devices;
	} catch (schemaError) {
		// Schema validation failed, but we already have devices extracted
		// This is okay - we'll use the raw data
		console.log("[Alexa] Using raw device data (schema validation skipped)");
	}
	
	console.log(`[Alexa] ✅ Successfully connected! Found ${devices.length} devices:`);
	devices.forEach((device: any, index: number) => {
		const status = device.online ? "🟢 ONLINE" : "🔴 OFFLINE";
		const name = device.accountName || device.deviceName || device.friendlyName || "Unknown";
		const type = device.deviceType || device.deviceFamily || "Unknown";
		console.log(`[Alexa]   ${index + 1}. ${name} (${type}) - ${status}`);
	});
...
export async function getCustomerSmartHomeEndpoints(env: Env) {
	...
	const cached = getCached<any[]>(cacheKey);
	if (cached) {
		console.log(`[Alexa] Using cached smart home endpoints (${cached.length} endpoints)`);
		return cached;
	}

	console.log("[Alexa] Fetching customer smart home endpoints via GraphQL...");
	...
	if (endpoints.length > 0) {
		console.log(`[Alexa] ✅ Found ${endpoints.length} smart home devices:`);
		endpoints.slice(0, 10).forEach((endpoint: any, index: number) => {
			const category = endpoint.displayCategories?.primary?.value || "UNKNOWN";
			console.log(`[Alexa]   ${index + 1}. ${endpoint.friendlyName} (${category})`);
		});
		if (endpoints.length > 10) {
			console.log(`[Alexa]   ... and ${endpoints.length - 10} more devices`);
		}
	} else {
		console.log("[Alexa] ⚠️  No smart home devices found");
	}
